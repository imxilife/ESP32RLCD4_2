...
WiFiConfig::WiFiConfig()
    : impl_(new Impl()), wifiConnected(false), messageCallback(nullptr) {
}
...
bool WiFiConfig::begin() {
    ...
    impl_->wifiManager.addParameter(&param_user);
    impl_->wifiManager.setDebugOutput(true);
    impl_->wifiManager.setAPCallback(configModeCallback);
    impl_->wifiManager.setConnectTimeout(3);       // PSK 快速超时（企业 WiFi 会失败，预期行为）
    impl_->wifiManager.setConfigPortalTimeout(0);  // 门户永不超时，等待用户操作
    impl_->wifiManager.setBreakAfterConfig(true);  // 提交表单后立即退出门户
    impl_->wifiManager.setSaveConfigCallback([this]() {
        emitMessage("WiFi连接中...", "");
    });

    String apName = "ESP32_RLCD_" + String((uint32_t)ESP.getEfuseMac(), HEX);
    impl_->wifiManager.startConfigPortal(apName.c_str());

    // 3. 读取门户提交的凭据
    String ssid = impl_->wifiManager.getWiFiSSID(true);
    String pass = impl_->wifiManager.getWiFiPass(true);
...
void WiFiConfig::reset() {
    impl_->wifiManager.resetSettings();
    // 同时清除本地保存的多网络凭据
    Preferences prefs;
    prefs.begin("wifi_creds", false);
    prefs.clear();
